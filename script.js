...
// Atualiza visualização do formulário e progress bar
...
function gerarPdf(formDataObj){
  ...
  function drawSectionTitle(title){
    ...
    doc.roundedRect(margin,y,150,titleBgHeight,6,6,'F');
    doc.setFont('helvetica','bold'); doc.setFontSize(12); doc.setTextColor(0,0,0);
    doc.text(title.toUpperCase(), margin+160, y+titleBgHeight/1.7); y+=titleBgHeight+titlePadding;
  }

  doc.setFontSize(18); doc.setFont('helvetica','bold');
  doc.text('FICHA DE AVALIAÇÃO', pageWidth/2, y, {align:'center'}); y+=30;
  ...
  addLines([
    ...
    `Idade: ${formDataObj.idade||'—'}`,
    `Peso: ${formDataObj.peso||'—'}`,
    `Altura: ${formDataObj.altura||'—'}`,
    `Endereço: ${formDataObj.endereco||'—'}`,
  ...
  addLines([
    `Acorda: ${formDataObj.acorda||'—'}`,
    `Trabalho: ${formDataObj.trabalho_inicio||'—'} até ${formDataObj.trabalho_fim||'—'}`,
    `Treino: ${formDataObj.treino_horario||'—'}`,
    `Dorme: ${formDataObj.dorme||'—'}`
  ]);

  drawSectionTitle('Alimentação');
  addLines([
    `Restrição/alergia: ${formDataObj.restricao_quais||'Não'}`,
    `Incluir alimentos: ${formDataObj.incluir_alimentos||'—'}`,
    `Não incluir: ${formDataObj.excluir_quais||'—'}`,
    `Frutas, legumes e verduras: ${formDataObj.preferencias_flv||'—'}`,
    `Alimentação atual: ${formDataObj.alimentacao_atual||'—'}`,
    `Leva marmita: ${formDataObj.marmita||'Não'}`
  ]);

  drawSectionTitle('Saúde & Exercícios');
  addLines([
    `Alergia: ${formDataObj.alergia_quais||'Não'}`,
    `Doença crônica: ${formDataObj.doenca_quais||'Não'}`,
    `Lesão: ${formDataObj.lesao_quais||'Não'}`,
    `Atividades aeróbicas: ${formDataObj.aerobico||'Não'}`,
    `Horário aeróbico: ${formDataObj.aerobico_horario||'—'}`,
    `Freq. treino/semana: ${formDataObj.freq_treino||'—'}`,
    `Tipo de treino: ${formDataObj.tipo_treino||'—'}`
  ]);

  drawSectionTitle('Suplementos & Protocolos');
  addLines([
    `Suplementos: ${formDataObj.suplemento_quais||'Não'}`,
    `Protocolo hormonal: ${formDataObj.hormonal_quais||'—'}`
  ]);
...
document.getElementById('multiStepForm').addEventListener('submit', async e=>{
  ...
  const pdfDataUri = gerarPdf(formDataObj);
  const base64PDF = pdfDataUri.split(',')[1]; // remove "data:application/pdf;base64,"

  try{
    await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
      to_email: 'diegossilva03@gmail.com',
      message: 'Ficha de avaliação em anexo',
      attachments: [{name: "Ficha_Avaliacao.pdf", data: base64PDF}]
    });
